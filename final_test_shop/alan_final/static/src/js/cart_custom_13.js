odoo.define('alan_final.cart_custom_13', function (require) {
    'use strict';
...
publicWidget.registry.WebsiteSale = publicWidget.Widget.extend(VariantMixin, {
    ...
    events: _.extend({}, VariantMixin.events || {}, {
        ...
        'click a.js_add_cart_json_custom': '_onClickAddCartCustomJSON',
    ...
    _changeCartQuantity: function ($input, value, $dom_optional, line_id, productIDs) {
        ...
        this._rpc({
            route: "/shop/cart_custom/update_json",
            params: {
                line_id: line_id,
            ...
            console.log($('.js_quantity').val())
            if(data.quantity === 0) {
                    const id_product = $input.closest(".devvn_sidebar_entry").data("product-cart");

                    $(".devvn_sidebar_entry[data-product-cart='"+id_product+"']").remove();

                    renderTemplateCart();
                }

                $('#cart_custom, #payment_custom').removeAttr("disabled");

                $('#cart_custom, #payment_custom').unbind('click').click(function(event) {
            ...
            if (!data.cart_quantity && url.replace(/^.*\/\/[^\/]+/, '') === '/shop/cart') {
                return window.location = '/shop';
            }
            if (!data.cart_quantity && url.replace(/^.*\/\/[^\/]+/, '') === '/shop/address') {
                return window.location = '/shop';
            }
            ...
            let total_quantity = data.cart_quantity || '';
            let total_amount = data.amount_total || 0;

            $('.devvn_fix_scroll').find('.my_cart_quantity').html(total_quantity);
            $('.devvn_cart_total').find('.my_cart_quantity').html(total_quantity);
            $('.dev_order_mobile').find('.quantity_monile').html(total_quantity);
            $('.devvn_cart_fulltotal').find('.my_cart_quantity_total').html(total_amount);
            $('.dev_order_mobile').find('.my_cart_quantity_total').html(total_amount);
            $(".js_cart_lines").first().before(data['website_sale.cart_lines']).end().remove();
            $('.my_cart_quantity_header').html(total_quantity);
            // $('.my_cart_quantity').html((total_quantity))
            if(!total_quantity) {
                $('.my_cart_quantity_header').remove();
            }
            if(total_quantity == 0){
    ...
    onClickAddCartCustomJSON: function (ev) {
        console.log("test1231");
        ev.preventDefault();
        var $link = $(ev.currentTarget);
        var $input = $link.closest('.shop_cart_quanty').find("input");
        console.log($input)
        var min = parseFloat($input.data("min") || 0);
        var max = parseFloat($input.data("max") || Infinity);
        var previousQty = parseFloat($input.val() || 0, 10);
        var quantity = ($link.has(".change_data").length ? -1 : 1) + previousQty;
    ...
    _onClickAddCartCustomJSON: function (ev){
        this.onClickAddCartCustomJSON(ev);
    },
    /**
     * @private
     * @param {Event} ev
...
publicWidget.registry.websiteSaleCart = publicWidget.Widget.extend({
    ...
    events: {
        ...
        'click .js_delete_product_shop': '_onClickDeleteProductShop',
    },
    ...
    _onClickDeleteProductShop: function (ev) {
        ev.preventDefault();
        swal({
            ...
            .then((willDelete) => {
                if (willDelete || $('#cart_products').find('.js_quantity').val() === 0) {
                    swal("Bạn đã xóa thành công", {
                        icon: "success",
                        buttons: $(ev.currentTarget).closest('#cart_products').find('.js_quantity').val(0).trigger('change'),
                    });
                } else {
